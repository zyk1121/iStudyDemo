...
//  LeadorMapSDK
//
//  Created by zhangyuanke on 16/3/9.
//  Copyright © 2016年 zhangyuanke. All rights reserved.
//

#import "LDDataExtension.h"
#include <zlib.h>
...
//	return [NSString stringWithCString:chars length:sizeof(chars)];
	return [[NSString alloc] initWithBytes:chars length:sizeof(chars) encoding:NSUTF8StringEncoding];
}
...
- (unsigned int)crc32
{
	...
	bytes = [self bytes];
	max = (unsigned)[self length];
	crcval = 0xffffffff;
	for (x = 0, y = max; x < y; x++) {
		crcval = (unsigned)(((crcval >> 8) & 0x00ffffff) ^ crc32table[(crcval ^ (*((unsigned char *)bytes + x))) & 0xff]);
	}
