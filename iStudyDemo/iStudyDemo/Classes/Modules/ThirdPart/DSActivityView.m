...
+ (DSActivityView *)activityViewForView:(UIView *)addToView withLabel:(NSString *)labelText width:(NSUInteger)labelWidth;
{
    ...
    return [[[self alloc] initForView:addToView withLabel:labelText width:labelWidth] autorelease] ;
}
...
{
	if (!addToView || !(self = [super initWithFrame:CGRectZero]))
		return nil;
...
{
    return [self activityViewWithLabel:NSLocalizedString(@"Loading...", @"Default DSActivtyView label text")];
}
...
#pragma mark -
// ----------------------------------------------------------------------------------------
@implementation MTNoneMaskBezelActivityView
